...
def add_player_to_game(player):
    ...
    if gamesList[len(gamesList)-1].number_of_players == 1:
        ...
        threading.Thread(gameplay(gamesList[len(gamesList)-1])).start()
...
def new_player_handler(client_socket):
    ...
    threading.Thread(client_send_handler(client_socket, player, current_game)).start()
    threading.Thread(client_recv_handler(client_socket,player,current_game)).start()
...
while True:
    ...
    try:
        ...
        threading.Thread(new_player_handler(client_socket)).start()
